...
describe Stats, "for Rails" do
  def app
    Stats::Middleware.new(sample_app, $stats)
  end
  ...
  describe "HTTP" do
    before do
      $stats = Stats.new([:uri, :http_method, :response_code], $log)
    end
  ...
  describe "routing" do
    before do
      $stats = Stats.new([:controller_name, :action_name], $log)
    end
  ...
  describe "SQL" do
    before do
      $stats = Stats.new([:sql_queries, :sql_usr_time, :sql_sys_time, :sql_real_time], $log)
    end
  ...
  describe "Memcached" do
    before do
      $stats = Stats.new([:memcache_writes, :memcache_misses, :memcache_hits, :memcache_usr_time, :memcache_sys_time, :memcache_real_time], $log)
    end
  ...
  describe "ActiveRecord" do
    before do
      $stats = Stats.new([:active_record_instances], $log)
    end
