if defined?(Memcached)
  Memcached.class_eval do
    ...
    def set_with_stats_log(key, value, timeout=0, marshal=true)
      Oink.broadcaster.incr(:memcache_writes)
      Oink.broadcaster.measure(:memcache) do
    ...
    def get_with_stats_log(keys, marshal=true)
      if keys.is_a? Array
        results = Oink.broadcaster.measure(:memcache) do
          get_without_stats_log(keys, marshal)
        end
        
        keys.each do |key|
          if results.has_key?(key)
            Oink.broadcaster.incr(:memcache_hits)
          else
            Oink.broadcaster.incr(:memcache_misses)
          end
        end
        ...
        begin
          result = Oink.broadcaster.measure(:memcache) do
            get_without_stats_log(keys, marshal)
          end
          Oink.broadcaster.incr(:memcache_hits)
          return result
